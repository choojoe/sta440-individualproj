...
$$
\begin{aligned}
\log (P_{ij}/1-P_{ij}) = \beta_{0j} + \beta_{1j}*Length_{ij} +  \beta_{2j}*Exclamations_{ij} + \beta_{3j}*Questions_{ij} + \beta_{4...49j}*(\text{Word})_{ij} + (\epsilon_i)\\

\\
\text{where P is probability of }  review_{i} \text{ having a rating level} _j \text{ };(\epsilon_i) \text{ are commonly assumed to be i.i.d. and follow some specified distribution}
\end{aligned}
$$
...
The 3 logistic regression models' significant coefficients were put into 1 table but the full table can be found in Appendix. At a high level, 25 predictors (excluding intercepts) were identified as significant (at $\alpha$ = .05) in the Negative logistic regression model, 21 in the Neutral, and 23 in the Positive. The predictor variables of: length, exclamations, questions, always, austin, checkin, friend, great, love, pretty, and seat were all significant across the three models. Because the model output is exponentiated, whenever the estimate is greater than 1, the odds that the response variable is the rating of interest increases. Similarly, when the estimate is less than 1, the odds that the response variable is the rating of interest decreases.
...
\newpage
## Appendix 1: Process of text analysis + terms 
...
Below are the 46 words with over 1000 instances
```{r, echo=FALSE}
...
## Appendix Model Assumptions for Logistic Regression
```{r}
...
3) Linear relationship between the logit of the outcome and each predictor variables: this assumption is satisfied as seen in the Predictor Value vs Logit of Outcome plot. Though there appears to be a slight polynomial relationship, the relationship is largely linear and thus we consider it satisfied.

4) No influential values (extreme values or outliers) in the continuous predictors: this assumption is satisfied as the top 3 most influential points have relatively low Cook's distances.
